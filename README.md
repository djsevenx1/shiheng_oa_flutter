...
| 模块 | 说明 |
|------|------|
| 启动页 | 时恒电子品牌 + 启动预热服务器连接 + 自动登录判断 |
| 登录 | 服务器地址 / 用户名 / 密码 / 加速 GitHub 代理配置 / 记住密码 / 自动登录 |
| 首页仪表盘 | 4 Tab 导航(仪表盘 / 工作流 / 报表 / 更多),下拉刷新,公告/待办/通知 |
| 流程审批 | 列表 / 详情 / 动态字段发起表单 / 审批操作 / 提交后回退 |
| 报表中心 | 报表分类 + 时恒专属报表 + 库存报表 |
